...
import { baseMime } from '../services/whatsappOutboundAudioService.js';
 ...
 * @returns {string | null}
 */
const normalizeImageContentType = (mimetype) => {
  let base = baseMime(mimetype);
  if (!base) return null;
  ...
  try {
    ...
    if (!base) {
      return res.status(503).json({
        success: false,
        error:
          'PUBLIC_BASE_URL is required so Twilio can fetch the image (same as voice messages)'
      });
    }
    ...
    const gridfsId = await uploadBuffer(file.buffer, {
      filename,
      metadata: {
        source: 'crm_outbound_image',
    ...
    const saved = await Message.create({
      ...
      message_type: captionText ? 'mixed' : 'image',
      direction: 'outgoing',
      deliveryStatus: 'sent',
      from: 'crm',
      to: normalizedPhone,
      body: captionText,
      message: captionText || '[Photo]',
      timestamp: now,
      createdAt: now,
    ...
    const msg =
      typeof err?.message === 'string' ? err.message : 'Failed to send image';
